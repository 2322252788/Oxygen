...
public class UIPopUPChooseButton {
    ...
    public void draw(int mouseX, int mouseY) {
        ...
        if (this.currentRadius > 1.0f) {
        	RenderUtil.circle(this.x, this.y, this.currentRadius, Colors.ASPHALT.c);
            if (this.isHovering(mouseX, mouseY)) {
        ...
        if (this.currentRadius == this.maxRadius && this.isHovering(mouseX, mouseY)) {
            ...
            RenderUtil.drawRoundedRect(xFont - 2, yFont - 1, xFont + font.getStringWidth(this.name) + 2, yFont + font.FONT_HEIGHT + 1, 1.0f, Colors.ASPHALT.c);
            font.drawString(this.name, (float)xFont, yFont, Colors.GREY.c);
        }
    }
