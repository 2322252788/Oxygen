...
public class UISlider {
    ...
    public double draw(final float value, final int mouseX, final int mouseY, final int x, final int y) {
        this.height = 2;
        final UnicodeFontRenderer font = Oxygen.INSTANCE.font.consolasbold12;
        ...
        Gui.drawRect(x, y, x + this.width, y + this.height,  new Color(Colors.GREY.c).brighter().brighter().getRGB());
        font.drawString(strValue, x + this.width / 2 - vWidth / 2.0f + 0.5f, y +2f + this.height + 2.5f, Colors.BLACK.c);
    ...
    private double changeValue(final float value, final int mouseX, final int mouseY, final int x, final int y) {
        ...
        RenderUtil.drawRect((float)x, (float)y, (this.lastMouseX == -1.0f) ? ((float)(int)val) : (x + this.width * this.lastMouseX), (float)(y + this.height), new Color(-14848033).brighter().getRGB());
        RenderUtil.circle((float)val, y + 1, 3f, new Color(-14848033).brighter().getRGB());
        if (this.drag) {
