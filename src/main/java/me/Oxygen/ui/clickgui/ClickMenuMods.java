...
import me.Oxygen.ui.clickgui.options.UIMode;
import me.Oxygen.ui.clickgui.options.UISlider;
import me.Oxygen.ui.clickgui.options.UIToggleButton;
...
public class ClickMenuMods {
    ...
    public void draw(final int mouseX, int mouseY) {
        ...
        final UnicodeFontRenderer font = Oxygen.INSTANCE.font.tahoma16;
        ...
        this.valueYAdd = 0;
        for (final Module m : this.modList) {
        	//单个Modlue渲染
            Gui.drawRect(this.x, yAxis, this.x + this.width, yAxis + height, -1);
            ...
            if (m.hasValues()) {
                ...
                GL11.glTranslated(-xMid, -yMid, 0.0);
                if (arrowHover) {
                    RenderUtil.drawImage(new ResourceLocation("Oxygen/icons/arrow-down.png"), this.x + this.width - 8, yAxis + (height - size) / 2 + 1, size, size, new Color(0.7058824f, 0.7058824f, 0.7058824f));
                } else {
                    RenderUtil.drawImage(new ResourceLocation("Oxygen/icons/arrow-down.png"), this.x + this.width - 8, yAxis + (height - size) / 2 + 1, size, size, new Color(0,0,0));
                }
                GL11.glPopMatrix();
            }
            ...
            font.drawString(m.getName(), (float)(this.x + (this.width - font.getStringWidth(name)) / 2) - 8, (float)(yAxis + (height - font.FONT_HEIGHT) / 2), m.isEnabled() ? 0 : Colors.GREY.c);
            if (m.openValues) {
    ...
    private void addMods() {
        for (final Module m : Oxygen.INSTANCE.ModMgr.getModules()) {
