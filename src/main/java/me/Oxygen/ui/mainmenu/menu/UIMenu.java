package me.Oxygen.ui.mainmenu.menu;
...
public class UIMenu {
   ...
   public void draw(int mouseX, int mouseY) {
      ...
      UnicodeFontRenderer font = Oxygen.INSTANCE.font.simpleton70;
      this.height = res.getScaledHeight();
      RenderUtil.drawRect((int)this.x, 0, (int)this.x + this.width + 2, this.height, -1);
      RenderUtil.drawRect(this.x, 0.0F, this.x + (float)this.width, (float)this.height, -14145496);
      if (this.x == (float)(-this.width)) {
         RenderUtil.drawImage(new ResourceLocation("Oxygen/mainmenu/slidermenu_icon.png"), (int)this.x + this.width + 5, res.getScaledHeight() / 2 - 3, 6, 6);
      }
      ...
      //RenderUtil.drawCircle((int)this.x + this.width / 2 + 1, 36, 15, new Color(255, 255, 255));
      int tab_height = 20;
      int yStart = res.getScaledHeight() / 2 - this.slots.size() / 2 * tab_height - 5;
      font = Oxygen.INSTANCE.font.simpleton25;
      ...
      for(Iterator var9 = this.slots.iterator(); var9.hasNext(); yStart += 21) {
         ...
         RenderUtil.drawRect(this.x + (float)((this.width - font.getStringWidth(slot.text)) / 2) - 2.0F, (float)(yStart + font.FONT_HEIGHT) + 1.5F, this.x + (float)((this.width - font.getStringWidth(slot.text)) / 2) - 2.0F + (float)((int)((double)font.getStringWidth(slot.text) * slot.animationX)), (float)(yStart + font.FONT_HEIGHT + 2), -7748364);
