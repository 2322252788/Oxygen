...
public class UITextField extends GuiTextField {
	...
	public void drawOutlinedRect(float x, float y, float width, float height, float lineSize, int lineColor,
			int backgroundColor) {
	...
	public void drawTextBox() {
		if (this.getVisible()) {
			if (this.getEnableBackgroundDrawing()) {
				drawOutlinedRect((float) this.xPosition, (float) this.yPosition, (float) (this.xPosition + this.width),
						(float) (this.yPosition + this.height), 0.7F,
						this.isFocused ? Colors.DARKBLUE.c : Colors.GREY.c, 0);
			}
			...
			if (s.length() > 0) {
				...
				j1 = (int) this.fontRendererInstance.drawString(flag2, (float) l, (float) i1, -1, false);
			}
			...
			if (flag1) {
				if (var13) {
					...
					this.fontRendererInstance.drawString("_", (float) k1, (float) i1, -1, false);
				}
			}

			if (k != j) {
				int l1 = l + this.fontRendererInstance.getStringWidth(s.substring(0, k));
				drawRect(k1, i1 - 1, l1 - 1, i1 + 1 + this.fontRendererInstance.FONT_HEIGHT,
						RenderUtil.reAlpha(Colors.DARKBLUE.c, 0.3F));
			}
		}
