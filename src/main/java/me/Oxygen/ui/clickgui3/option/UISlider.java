...
public class UISlider
{
    ...
    public void draw(final float x, final float y) {
        this.x = x;
        this.y = y;
        final UnicodeFontRenderer tahoma20 = Oxygen.INSTANCE.font.tahoma20;
        ...
        if (this.showValue) {
        	Oxygen.INSTANCE.font.tahoma15.drawString(this.value.getValueState() + "", x - 78.0f - tahoma20.getStringWidth(this.value.getValueState() + ""), y, Colors.GREY.c);
        }
        if (this.ani == 0.0f) {
            this.ani = (float)(x - 15.0f - (60.0 - 60.0 * n));
        }
        this.ani = (float) RenderUtil.getAnimationState(this.ani, (x - 15.0f - (60.0 - 60.0 * n)), Math.max(10.0, Math.abs(this.ani - (x - 15.0f - (60.0 - 60.0 * n))) * 30.0 * 0.3));
        RenderUtil.drawRoundedRect(x - 75.0f, y + 3.0f, x - 15.0f, y + 6.0f, 1.0f, new Color(Colors.GREY.c).brighter().brighter().getRGB());
        RenderUtil.drawRoundedRect(x - 75.0f, y + 3.0f, this.ani, y + 6.0f, 1.0f, new Color(-14848033).brighter().getRGB());
    ...
    public void onPress(final int tx, final int ty) {
        ...
        if (this.isDraging && !Oxygen.INSTANCE.crink.menu.isDraggingSlider) {
        	Oxygen.INSTANCE.crink.menu.isDraggingSlider = true;
        }
        else {
        	Oxygen.INSTANCE.crink.menu.isDraggingSlider = false;
        }
        this.tX = tx;
        this.tY = ty;
    }
