...
import me.Oxygen.ui.clickgui3.option.UISlider;
import me.Oxygen.ui.clickgui3.option.UIToggleButton;
...
public class ClickMenu
{
    ...
    public ClickMenu() {
        ...
        this.fileDir = String.valueOf(this.mc.mcDataDir.getAbsolutePath()) + "/" + Oxygen.INSTANCE.CLIENT_NAME;
    ...
    public void draw(int n, int n2) {
        ...
        if (this.settingMode && this.currentMod != null) {
            RenderUtil.drawRect(0.0f, 0.0f, n3, n4, RenderUtil.reAlpha(Colors.BLACK.c, 0.45f));
            ...
            for (Value values : Value.list) {
                if (values.getValueName().split("_")[0].equalsIgnoreCase(this.currentMod.getName())) {
                    Module currentMod = this.currentMod;
            ...
            Oxygen.INSTANCE.font.tahoma30.drawString(this.currentMod.getName(), n3 / 2.0f - 100.0f, n4 / 2.0f - 135.0f - Oxygen.INSTANCE.font.tahoma30.FONT_HEIGHT, Colors.WHITE.c);
            RenderUtil.drawRoundedRect(n3 / 2.0f - 100.0f, n4 / 2.0f - 130.0f, n3 / 2.0f + 100.0f, n4 / 2.0f + 130.0f, 5.0f, Colors.WHITE.c);
            ...
            for (Value values2 : this.modBooleanValue) {
                ...
                Oxygen.INSTANCE.font.tahoma20.drawString(values2.getValueName().split("_")[1], n3 / 2.0f - 94.0f, currentStartY - 2.0f, Colors.BLACK.c);
                Object o;
            ...
            for (Value currentSelectingMode : this.modModeValue) {
            	Oxygen.INSTANCE.font.tahoma20.drawString(currentSelectingMode.getModeTitle(), n3 / 2.0f - 94.0f, currentStartY - 2.0f, Colors.BLACK.c);
                try {
                	Oxygen.INSTANCE.font.tahoma16.drawString(currentSelectingMode.getModeAt(currentSelectingMode.getCurrentMode()), n3 / 2.0f + 28.0f, currentStartY, Colors.DARKGREY.c);
                }
                catch (Exception ex) {
                    Module.sendMessage("\u93c4\u5267\u305a\u93c3\u8dfa\u5f42\u9422\u71bc\u654a\u7487\ufffd,\u5bb8\u8336\ue195\u7f03\ue1bb\u8d1f\u699b\u6a3f\ue17b\u59af\u2033\u7d21");
                    currentSelectingMode.setCurrentMode(0);
                }
                if (!this.isSelectingMode) {
                	Oxygen.INSTANCE.font.tahoma25.drawString(">", n3 / 2.0f + 84.0f, currentStartY - 4.0f, new Color(Colors.GREY.c).brighter().getRGB());
                ...
                if (this.isSelectingMode) {
                    if (this.currentSelectingMode == currentSelectingMode) {
                    	Oxygen.INSTANCE.font.tahoma25.drawString("-", n3 / 2.0f + 87.0f, currentStartY - 4.0f, new Color(Colors.GREY.c).brighter().getRGB());
                        this.currentStartY = currentStartY;
                    }
                    else {
                    	Oxygen.INSTANCE.font.tahoma25.drawString(">", n3 / 2.0f + 84.0f, currentStartY - 3.0f, new Color(Colors.GREY.c).brighter().getRGB());
                    }
                }
                currentStartY += 18.0f;
            }
            ...
            if (this.isSelectingMode && this.currentSelectingMode != null) {
                ...
                while (i < currentSelectingMode2.mode.size()) {
                	Oxygen.INSTANCE.font.tahoma15.drawString((String)currentSelectingMode2.mode.get(i), n3 / 2.0f + 106.0f, currentStartY2 + 3.0f, Colors.BLACK.c);
    ...
    public void mouseClick(int n, int n2) {
        Iterator<me.Oxygen.ui.clickgui3.button.Button> iterator = booleanValueMap.values().iterator();
        while (iterator.hasNext()) {
