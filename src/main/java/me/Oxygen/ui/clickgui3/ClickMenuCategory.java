...
public class ClickMenuCategory
{
    ...
    public void draw(int p_draw_1_, int p_draw_2_) {
        ...
        UnicodeFontRenderer tahoma18 = Oxygen.INSTANCE.font.tahoma18;
        String string = this.c.name().substring(0, 1) + this.c.name().toLowerCase().substring(1, this.c.name().length());
        RenderUtil.drawRect(this.x, this.y, (this.x + this.width), (this.y + this.tab_height), RenderUtil.reAlpha(-658186, 0.95f));
        ...
        if (this.uiMenuMods.open) {
            RenderUtil.drawGradientRect(this.x, this.y + this.tab_height - 2, this.x + this.width, this.y + this.tab_height + 3, RenderUtil.reAlpha(Colors.BLACK.c, 0.3f), 0);
        }
