...
import me.Oxygen.api.CloudMusicAPI;
import me.Oxygen.api.NeteaseAPI;
...
public class MusicWindow extends GuiScreen {
	...
	public MusicManager mgr = MusicManager.instance;
	...
	public void initGui() {
		...
		for (SongList s : MusicManager.instance.allSongLists) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		RenderUtil.drawRoundedRect(x, y + 20, x + 510, y + 335, RenderUtil.reAlpha(Colors.WHITE.c, 0.85f), ClientUtil.reAlpha(Colors.WHITE.c, 0.85f));
		RenderUtil.drawRoundedRect(x, y, x + 510, y + 20, RenderUtil.reAlpha(Colors.AQUA.c, 0.85f), ClientUtil.reAlpha(Colors.AQUA.c, 0.85f));
		RenderUtil.drawRect(x, y + 19, x + 1, y + 20, RenderUtil.reAlpha(Colors.AQUA.c, 0.85f));
		RenderUtil.drawRect(x + 509, y + 19, x + 510, y + 20, RenderUtil.reAlpha(Colors.AQUA.c, 0.85f));
		
		RenderUtil.drawRect(x, y + 20, x + 1, y + 21, RenderUtil.reAlpha(Colors.WHITE.c, 0.85f));
		RenderUtil.drawRect(x + 509, y + 20, x + 510, y + 21, RenderUtil.reAlpha(Colors.WHITE.c, 0.85f));

		if(this.currentWindow == WindowCategory.MUSIC) {
			RenderUtil.drawRect(x + 8, y, x + 44, y + 20, new Color(Colors.AQUA.c).darker().getRGB());
		} else {
			RenderUtil.drawRect(x + 47, y, x + 46 + Oxygen.INSTANCE.font.wqy16.getStringWidth(mgr.nickname) + 4, y + 20, new Color(Colors.AQUA.c).darker().getRGB());
		}
		
		Oxygen.INSTANCE.font.wqy16.drawString("我的歌单", (float) x + 10, y + 5, Colors.WHITE.c);

		if (mgr.isLoggined && mgr.user_avatar != null) {
			Oxygen.INSTANCE.font.wqy16.drawString(mgr.nickname, (float) x + 48, y + 5, Colors.WHITE.c);
		} else {
			mgr.nickname = "登录";
			Oxygen.INSTANCE.font.wqy16.drawString(mgr.nickname, (float) x + 48, y + 5, Colors.WHITE.c);
		}

		if (RenderUtil.isHovering(mouseX, mouseY, x + 48, y, x + 48 + Oxygen.INSTANCE.font.wqy16.getStringWidth(mgr.nickname) + 4, y + 20)) {
		...
		if (this.currentWindow == WindowCategory.USER) {
			if (mgr.isLoggined) {
				...
				Oxygen.INSTANCE.font.wqy16.drawString("欢迎, " + mgr.nickname, x + 86f, y + 25, Colors.BLACK.c);
				Oxygen.INSTANCE.font.wqy16.drawString("所在城市区域代码: " + mgr.location_code, x + 86f, y + 35f,
						Colors.BLACK.c);
				Oxygen.INSTANCE.font.wqy16.drawString("关注: " + mgr.follows + " 粉丝: " + mgr.followeds, x + 86f,
						y + 45, Colors.BLACK.c);

				// 歌曲搜索部分
				Oxygen.INSTANCE.font.tahoma25.drawString("Search", x + 20f, y + 92, Colors.BLACK.c);
				
				this.searchField.xPosition = x + 63;
				this.searchField.yPosition = y + 86;
				this.searchField.drawTextBox();
				
				RenderUtil.drawRect(x + 20, y + 90 + Oxygen.INSTANCE.font.tahoma30.FONT_HEIGHT, x + 234,
						y + 91 + Oxygen.INSTANCE.font.tahoma30.FONT_HEIGHT, Colors.BLACK.c);
				
				if (MusicManager.instance.searchResult.isEmpty()) {
					Oxygen.INSTANCE.font.wqy16.drawString("你啥都没搜索", x + 20f, y + 108, Colors.BLACK.c);
				} else {
					this.processSearchScroll(mouseX, mouseY);
					GL11.glEnable(GL11.GL_SCISSOR_TEST);
					RenderUtil.doGlScissor(x + 20, y + 110, 214, 222);
					wheelSearchSmoothTrack = (float) RenderUtil.getAnimationState(wheelSearchSmoothTrack, wheelSearchStateTrack * 20f,
					...
					for (TrackSlot s : mgr.searchResult) {
						...
						if(RenderUtil.isHovering(mouseX, mouseY, x + 24 + 175,(int) startY1, x + 24 + 210, (int)startY1 + 24)) {
							if(this.handler.canExcecute()) {
								if(MusicManager.instance.lastPlayingTrack == null) {
									MusicManager.instance.lastPlayingTrack = mgr.searchResult;
								}
				...
				// 日推
				Oxygen.INSTANCE.font.tahoma25.drawString("Daily", x + 260f, y + 92, Colors.BLACK.c);
				RenderUtil.drawRect(x + 260, y + 90 + Oxygen.INSTANCE.font.tahoma30.FONT_HEIGHT, x + 490,
						y + 91 + Oxygen.INSTANCE.font.tahoma30.FONT_HEIGHT, Colors.BLACK.c);
				RenderUtil.drawRoundedRect(x + 500 - 40, y + 92, x + 490, y + 105, Colors.AQUA.c, Colors.AQUA.c);
				Oxygen.INSTANCE.font.wqy16.drawString("获取", x + 505 - 35f, y + 94.5f, Colors.WHITE.c);
				if(RenderUtil.isHovering(mouseX, mouseY, x + 500 - 40, y + 92, x + 490, y + 105)) {
				...
				if(!mgr.dailyList.isEmpty()) {
					...
					wheelDailySmoothTrack = (float) RenderUtil.getAnimationState(wheelDailySmoothTrack, wheelDailyStateTrack * 20f, (Math.max(10, (Math.abs(this.wheelDailySmoothTrack - (wheelDailyStateTrack * 20))) * 50) * 0.3f));
					...
					for (TrackSlot s : mgr.dailyList) {
						...
						if(RenderUtil.isHovering(mouseX, mouseY, x + 264f + 186, c, x + 264f + 226, c + 24)) {
							if(this.handler.canExcecute()) {
								if(MusicManager.instance.lastPlayingTrack == null) {
									MusicManager.instance.lastPlayingTrack = mgr.dailyList;
								}
				...
				// 未登陆
				Oxygen.INSTANCE.font.wqy16.drawCenteredString("您还没有登录,请先登录", x + (510f / 2) - 5, y + 100,
						Colors.BLACK.c);
				...
				if (this.phoneNum.getText().isEmpty()) {
					Oxygen.INSTANCE.font.wqy16.drawString("电话号码", (x + (510f / 2) - 54), y + 128, Colors.GREY.c);
				}

				if (this.passwd.getText().isEmpty()) {
					Oxygen.INSTANCE.font.wqy16.drawString("密码", (x + (510f / 2) - 54), y + 148, Colors.GREY.c);
				}
				...
				Oxygen.INSTANCE.font.wqy16.drawString("登入", (x + (510f / 2) - 16), y + 195, Colors.WHITE.c);
		...
		if (this.currentWindow == WindowCategory.MUSIC) {
			...
			Oxygen.INSTANCE.font.wqy16.drawString("保存的歌单 (" + this.slots.size() + ")", x + 2f, y + 21,
					Colors.GREY.c);

			RenderUtil.drawRoundedRect(x + 122 - 26, y + 22, x + 118, y + 32f, Colors.AQUA.c, Colors.AQUA.c);
			Oxygen.INSTANCE.font.wqy16.drawString("刷新", x + 122f - 23f, y + 21, Colors.WHITE.c);

			if (RenderUtil.isHovering(mouseX, mouseY, x + 122 - 26, y + 22, x + 118, y + 32f)) {
				if (this.handler.canExcecute()) {
					if(mgr.isLoggined) {
						MusicManager.instance.currentSongList = null;
						this.slots.clear();
						...
						ClientUtil.tellPlayer("§e[MusicPlayer]§f请先登录");
					}
				}
			}
			
			if(mgr.currentTrack != null) {
				if (MusicManager.instance.trackImage.containsKey(mgr.currentTrack.getId())) {
					GL11.glPushMatrix();
				...
				Oxygen.INSTANCE.font.wqy16.drawString(mgr.currentTrack.getName(), x + 34f, y + 282, Colors.BLACK.c);
				Oxygen.INSTANCE.font.wqy16.drawString(mgr.currentTrack.getArtists(), x + 34f, y + 295, Colors.GREY.c);
				GL11.glDisable(GL11.GL_SCISSOR_TEST);
				GL11.glPopMatrix();
			...
			if (!this.slots.isEmpty()) {
				...
				wheelSongListSmoothTrack = (float) RenderUtil.getAnimationState(wheelSongListSmoothTrack, wheelSongListStateTrack * 20f,
			...
			if (MusicManager.instance.currentSongList != null) {

				SongList lists = MusicManager.instance.currentSongList;

				wheelSmoothTrack = (float) RenderUtil.getAnimationState(wheelSmoothTrack, wheelStateTrack * 20f,
				...
				if(!lists.songs.isEmpty()) {
						...
						if (startY1 < y + 320 && startY1 > y - 10) {
							...
							if (RenderUtil.isHovering(mouseX, mouseY, x + 126 + 350, startY1, x + 126 + 382, startY1 + 24) && this.handler.canExcecute()) {
								if(RenderUtil.isHovering(mouseX, mouseY, x + 122, y + 22, x + 121 + 387, y + 22 + 286)) {
									...
									if (MusicManager.instance.lastPlayingTrack == null
											|| MusicManager.instance.lastPlayingTrack != lists.songs) {
										MusicManager.instance.lastPlayingTrack = lists.songs;
									}
								}
							}
					...
					Oxygen.INSTANCE.font.wqy16.drawString("为防止请求太过频繁,请手动获取歌单内的歌曲", x + 232f, y + 120, Colors.BLACK.c);
					if(RenderUtil.isHovering(mouseX, mouseY, x + 232f, y + 132, x + 232 + Oxygen.INSTANCE.font.wqy16.getStringWidth("为防止请求太过频繁,请手动获取歌单内的歌曲"), y + 152)) {
						RenderUtil.drawRoundedRect(x + 232f, y + 132, x + 232 + Oxygen.INSTANCE.font.wqy16.getStringWidth("为防止请求太过频繁,请手动获取歌单内的歌曲"), y + 152, new Color(Colors.AQUA.c).darker().getRGB(), new Color(Colors.AQUA.c).darker().getRGB());
						if(this.handler.canExcecute()) {
							try {
						...
						RenderUtil.drawRoundedRect(x + 232f, y + 132, x + 232 + Oxygen.INSTANCE.font.wqy16.getStringWidth("为防止请求太过频繁,请手动获取歌单内的歌曲"), y + 152, Colors.AQUA.c, Colors.AQUA.c);
					}
					Oxygen.INSTANCE.font.wqy16.drawString("获取", x + 305f, y + 136, Colors.WHITE.c);
				}
			...
			if(mgr.mediaPlayer != null) {
				Oxygen.INSTANCE.font.wqy16.drawString(mgr.mediaPlayer.getStatus() == MediaPlayer.Status.PLAYING ? "| |" : "|>", x + 12f, y + 317, Colors.WHITE.c);
			} else {
				Oxygen.INSTANCE.font.wqy16.drawString("|>", x + 12f, y + 317, Colors.WHITE.c);
			}
			...
			Oxygen.INSTANCE.font.wqy16.drawString(">", x + 38f, y + 317, Colors.WHITE.c);
			
			//进度条
			double progress = 0;
			double progress2 = 0;
			if(mgr.mediaPlayer != null) {
				progress = MusicManager.instance.mediaPlayer.getCurrentTime().toSeconds();
				progress2 = MusicManager.instance.mediaPlayer.getStopTime().toSeconds();
				Oxygen.INSTANCE.font.tahoma12.drawString(mgr.formatSeconds((int)progress), x + 60f, y + 314f, Colors.BLACK.c);
				Oxygen.INSTANCE.font.tahoma12.drawString(mgr.formatSeconds((int)progress2), x + 420f - Oxygen.INSTANCE.font.tahoma12.getStringWidth(mgr.formatSeconds((int)progress2)), y + 314f, Colors.BLACK.c);
			}
			...
			//歌词显示
			Oxygen.INSTANCE.font.wqy16.drawStringWithShadow("词", x + 430f, y + 318, mgr.displayLyric == false ? Colors.GREY.c : Colors.AQUA.c);
			if(RenderUtil.isHovering(mouseX, mouseY, x + 430f, y + 320, x + 448f, y + 328)) {
				 if(this.handler.canExcecute()) {
			...
			//歌词编码
			Oxygen.INSTANCE.font.wqy16.drawStringWithShadow(mgr.isUtf ? "U" : "G", x + 450f, y + 318, Colors.GREY.c);
			if(RenderUtil.isHovering(mouseX, mouseY, x + 450f, y + 320, x + 468f, y + 328)) {
				if(this.handler.canExcecute()) {
					mgr.isUtf = !mgr.isUtf;
					ClientUtil.tellPlayer("§e[MusicPlayer]§f重新播放歌曲使其生效");
				}
			}
			//单曲循环
			Oxygen.INSTANCE.font.wqy16.drawStringWithShadow("R", x + 470f, y + 318, mgr.loop ? Colors.AQUA.c : Colors.GREY.c);
			if(RenderUtil.isHovering(mouseX, mouseY, x + 470f, y + 320, x + 488f, y + 323)) {
					...
					if(mgr.mediaPlayer != null) {
						if(mgr.loop) {
							mgr.mediaPlayer.setOnEndOfMedia(new Runnable() {
								@Override
								public void run() {
									if(MusicManager.instance.loop) {
				                		MusicManager.instance.play(MusicManager.instance.currentTrack);
				                	} else {
			...
			//RenderUtil.drawRoundedRect(this.x + 496, (this.y + 318), (this.x + 508), (this.y + 332), 2.0f, Colors.GREY.c);
			Oxygen.INSTANCE.font.comfortaa20.drawStringWithShadow("-", x + 500f, y + 318, Colors.GREY.c);
            if (RenderUtil.isHovering(mouseX, mouseY, this.x + 496, this.y + 318, this.x + 508, this.y + 332) && this.handler2.canExcecute()) {
            	//ClientUtil.tellPlayer("§e[MusicPlayer]§f音量-");
            	MusicManager.instance.mediaPlayer.setVolume(Math.max(0.1, MusicManager.instance.mediaPlayer.getVolume() - 0.1));
                MusicManager.instance.setVolume(MusicManager.instance.mediaPlayer.getVolume());
            }
            //RenderUtil.drawRoundedRect(this.x + 482, (this.y + 318), (this.x + 495), (this.y + 332), 2.0f, Colors.GREY.c);
            Oxygen.INSTANCE.font.comfortaa20.drawStringWithShadow("+", x + 485f, y + 318, Colors.GREY.c);
            if (RenderUtil.isHovering(mouseX, mouseY, this.x + 482, this.y + 318, this.x + 495, this.y + 332) && this.handler2.canExcecute()) {
            	//ClientUtil.tellPlayer("§e[MusicPlayer]§f音量+");
            	MusicManager.instance.mediaPlayer.setVolume(Math.max(0.1, MusicManager.instance.mediaPlayer.getVolume() + 0.1));
                MusicManager.instance.setVolume(MusicManager.instance.mediaPlayer.getVolume());
            }
	...
	public void processScroll(int mouseX, int mouseY) {
		if (RenderUtil.isHovering(mouseX, mouseY, x + 121, y + 20, x + 121 + 389, y + 310)) {
			if (mouseWheel > 0) {
				...
				if (wheelStateTrack * 10 > MusicManager.instance.currentSongList.songs.size() * -25f)
					wheelStateTrack--;
			}
		}
	}
	...
	public void processSearchScroll(int mouseX, int mouseY) {
		if (RenderUtil.isHovering(mouseX, mouseY, x + 20, y + 110, x + 20 + 160, y + 110 + 222)) {
			if (mouseWheel > 0) {
				...
				if (wheelSearchStateTrack * 10 > MusicManager.instance.searchResult.size() * -25f)
					wheelSearchStateTrack--;
			}
		}
	}
	...
	public void processDailyScroll(int mouseX, int mouseY) {
		if (RenderUtil.isHovering(mouseX, mouseY, x + 260f, y + 110, x + 490, y + 332)) {
			if (mouseWheel > 0) {
				...
				if (wheelDailyStateTrack * 10 > MusicManager.instance.dailyList.size() * -25f)
					wheelDailyStateTrack--;
			}
		}
	}
	...
	public void processSongListScroll(int mouseX, int mouseY) {
		if (RenderUtil.isHovering(mouseX, mouseY, x, y + 34, x + 120, y + 275)) {
			if (mouseWheel > 0) {
				...
				if (wheelSongListStateTrack * 10 > MusicManager.instance.allSongLists.size() * -25f)
					wheelSongListStateTrack--;
			}
		}
	}
	...
	public void moveWindow(int mouseX, int mouseY) {
		...
		if(RenderUtil.isHovering(mouseX, mouseY, x + 8, y, x + 44, y + 20) || RenderUtil.isHovering(mouseX, mouseY, x + 47, y, x + 46 + Oxygen.INSTANCE.font.wqy16.getStringWidth(mgr.nickname) + 4, y + 20)) {
			return;
		}
