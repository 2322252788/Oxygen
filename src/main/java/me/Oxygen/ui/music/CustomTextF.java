...
public class CustomTextF extends Gui {
	...
	public CustomTextF(int componentId, FontRenderer fontrendererObj, int x, int y, int par5Width, int par6Height,
	...
	public void drawTextBox() {
		if (this.getVisible()) {
			if (this.getEnableBackgroundDrawing()) {
				RenderUtil.drawRect(this.xPosition - 1, this.yPosition + this.height, this.xPosition + this.width + 1,
						this.yPosition + this.height + 1, new Color(0xFFE8E7E8).darker().getRGB());
			}

			int i = this.isEnabled ? Colors.BLACK.c : this.disabledColor;
			int j = this.cursorPosition - this.lineScrollOffset;
			int k = this.selectionEnd - this.lineScrollOffset;
			String s = this.fontRendererInstance.trimStringToWidth(this.isPasswordField ? this.text.replaceAll(".", "*").substring(this.lineScrollOffset) :this.text.substring(this.lineScrollOffset),
					this.getWidth());
			boolean flag = j >= 0 && j <= s.length();
			boolean flag1 = this.isFocused && this.cursorCounter / 6 % 2 == 0 && flag;
			float l = this.enableBackgroundDrawing ? this.xPosition : this.xPosition;
			int i1 = this.enableBackgroundDrawing ? this.yPosition + (this.height - 2) / 2 : this.yPosition;
			float j1 = l;
			...
			if (flag1) {
				if (flag2) {
					RenderUtil.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + this.fontRendererInstance.FONT_HEIGHT, -3092272);
				} else {
					this.fontRendererInstance.drawString("_", (float) k1 + 0.5f, (float) i1 + 2, i, false);
				}
			}

			if (k != j) {
				float l1 = l + this.fontRendererInstance.getStringWidth(s.substring(0, k));
				RenderUtil.drawRect(k1, i1 - 1, l1 - 1, i1 + 1 + this.fontRendererInstance.FONT_HEIGHT, Colors.GREY.c);
			}
		}
	}
