...
import me.Oxygen.ui.music.TrackSlot;
import me.Oxygen.utils.ClientUtil;
import me.Oxygen.utils.music.Lyric;
...
public class MusicManager {
	...
	public MusicManager() {
		...
		musicFolder = new File(String.valueOf(Minecraft.getMinecraft().mcDataDir.toString()) + File.separator
				+ Oxygen.INSTANCE.CLIENT_NAME + File.separator + "music");
		if (!musicFolder.exists()) {
			musicFolder.mkdirs();
		}
		
		imageFolder = new File(String.valueOf(Minecraft.getMinecraft().mcDataDir.toString()) + File.separator
				+ Oxygen.INSTANCE.CLIENT_NAME + File.separator + "image");
	...
	public void loadSomeShit() {
		if(this.isLoggined) {
			try {
				String result = NeteaseAPI.INSTANCE.getPlaylist(this.cookies, this.uid, 300, 0);
				//解析Json
			...
			ClientUtil.tellPlayer("§e[MusicPlayer]§f请先登录账号");
		}
	}
	...
	public void getTrackImage(Track track) {
		try {	
			...
			IImageBuffer iib2 = new IImageBuffer() {
				...
				@Override
				public void skinAvailable() {
					MusicManager.instance.trackImage.put(track.getId(), rl2);
				}
			};
			...
			try {
				...
				if (lyrics[0] != "") {
					if (lyrics[0].equalsIgnoreCase("NO LYRIC")) {
						...
						MusicManager.instance.lyric = CloudMusicAPI.INSTANCE.analyzeLyric(lyrics[0]);
					}
				} else {
					currentLyric = "(\u53d1\u751f\u9519\u8bef\u6216\u6b4c\u66f2\u4e0d\u5305\u542b\u6b4c\u8bcd)";
					MusicManager.instance.lyric.clear();
				}
				...
				if (lyrics[1] != "") {
					...
					if (lyrics[1].equalsIgnoreCase("NO LYRIC")) {
						...
						MusicManager.instance.translateLyric = CloudMusicAPI.INSTANCE.analyzeLyric(lyrics[1]);
					}

				} else {
					currentTranslateLyric = "(\u83b7\u53d6\u65f6\u51fa\u73b0\u9519\u8bef\u6216\u8bd1\u6587\u4e0d\u5b58\u5728)";
					MusicManager.instance.translateLyric.clear();
				}
	...
	public void updateLyric(Event event) {
