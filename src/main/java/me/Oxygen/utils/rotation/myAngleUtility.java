...
public class myAngleUtility {
    ...
    public static double isInFov(float var0, float var1, double var2, double var4, double var6) {
        ...
        float[] var9 = myAngleUtility.getAngleBetweenVecs(new Vec3(Minecraft.getMinecraft().thePlayer.posX, Minecraft.getMinecraft().thePlayer.posY, Minecraft.getMinecraft().thePlayer.posZ), new Vec3(var2, var4, var6));
    ...
    public static float[] getAngles(Entity entity) {
        ...
        double var1 = entity.posX - Minecraft.getMinecraft().thePlayer.posX;
        double var3 = entity.posZ - Minecraft.getMinecraft().thePlayer.posZ;
        if (entity instanceof EntityLivingBase) {
            EntityLivingBase var7 = (EntityLivingBase)entity;
            var5 = var7.posY + ((double)var7.getEyeHeight() - 0.4) - (Minecraft.getMinecraft().thePlayer.posY + (double)Minecraft.getMinecraft().thePlayer.getEyeHeight());
        } else {
            var5 = (entity.getEntityBoundingBox().minY + entity.getEntityBoundingBox().maxY) / 2.0 - (Minecraft.getMinecraft().thePlayer.posY + (double)Minecraft.getMinecraft().thePlayer.getEyeHeight());
        }
    ...
    public static boolean isValidToRotate(double var0, double var2) {
        if (Minecraft.getMinecraft().thePlayer != null && Minecraft.getMinecraft().theWorld != null && Minecraft.getMinecraft().thePlayer.getEntityWorld() != null) {
            Entity var5;
            Iterator var4 = Minecraft.getMinecraft().thePlayer.getEntityWorld().loadedEntityList.iterator();
            do {
                if (var4.hasNext()) continue;
                return false;
            } while (!((var5 = (Entity)var4.next()) instanceof EntityPlayer) || var5 == Minecraft.getMinecraft().thePlayer || (double)Minecraft.getMinecraft().thePlayer.getDistanceToEntity(var5) >= var0 || myAngleUtility.isInFov(Minecraft.getMinecraft().thePlayer.rotationYaw, Minecraft.getMinecraft().thePlayer.rotationPitch, var5.posX, var5.posY, var5.posZ) >= var2);
            return true;
        }
        return false;
    }
    ...
    private double getAngleYaw(EntityLivingBase var1) {
        return myAngleUtility.getAnglesIgnoringNull((Entity)var1, Minecraft.getMinecraft().thePlayer.rotationYaw, Minecraft.getMinecraft().thePlayer.rotationPitch)[0];
    }
}
