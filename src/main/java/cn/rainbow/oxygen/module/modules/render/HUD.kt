...
@Native
@ModuleInfo(name = "HUD", category = Render)
class HUD: Module() {
    ...
    @EventTarget(events = [RenderGuiEvent::class])
    private fun onEvent(event: Event) {
        if (event is RenderGuiEvent && !this.mc.gameSettings.showDebugInfo) {
            val sr = event.sr
            ...
            if (!logoMode.isCurrentMode("None")) {
                ...
                Oxygen.INSTANCE.fontmanager.wqy40.drawString(Oxygen.name.uppercase(), 10.0f, 5.0f, logoColor)
                ...
                Oxygen.INSTANCE.fontmanager.wqy20.drawString("X:$x1 Y:$y1 Z:$z1", 89.0f / 2.0f - 39.0f, 43.0f, logoColor)
                Oxygen.INSTANCE.fontmanager.wqy20.drawString("FPS:" + Minecraft.getDebugFPS(), 89.0f / 2.0f - 39.0f, 55.0f, logoColor)
            }
