...
public enum ESP2D {
    ...
    private final ESP esp = (ESP) Oxygen.INSTANCE.moduleManager.getModule(ESP.class);
    ...
    public void renderBox(Minecraft mc) {
        ...
        for (Entity entity : this.entityConvertedPointsMap.keySet()) {
            ...
            if ((double) mc.thePlayer.getDistanceToEntity(entity) < 2.5 && renderPositionsTop1[1] < 0.0) {
                shouldRender = false;
            }
            if (!shouldRender)
                continue;
            ...
            if (EntityUtils.isSelected(entity, false)) {
                ...
                try {
                    ...
                    final int color = ColorUtils.fadeBetween(ESP.espColours.getRGB(), ESP.espColours.getRGB());

                    if(esp.box.getCurrentValue()) {
                    ...
                    if (esp.health.getCurrentValue()) {
                        ...
                        Color customColor = health >= 0.0f ? blendColors(fractions, colors, progress).brighter()
                                : Color.RED;
                        if(esp.healthColor.isCurrentMode("Static"))
                            customColor = ESP.healthColors;
                        ...
                        if ((int) this.getIncremental(progress * 100.0f, 1.0) <= 100) {
                            ...
                            if (esp.tags.getCurrentValue()) {
                                ...
                                OFontRenderer tahoma13 = Oxygen.INSTANCE.fontmanager.tahoma13;
                    ...
                    ItemStack stack1 = ent.getEquipmentInSlot(4);
                    if (esp.armor.getCurrentValue()) {
                        if (stack1 != null) {
    ...
    public void updatePositions(Minecraft mc) {
        ...
        for (Entity e2 : mc.theWorld.getLoadedEntityList()) {
            if (!(e2 instanceof EntityLivingBase) || e2 == mc.thePlayer)
                continue;
            EntityLivingBase ent = (EntityLivingBase) e2;
